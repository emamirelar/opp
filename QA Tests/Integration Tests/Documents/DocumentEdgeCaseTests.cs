...
public class DocumentEdgeCaseTests : IClassFixture<PAOWebApplicationFactory<Program>>
{
    ...
    {
        var response = await _client.GetAsync("/api/document/download/1");
        response.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.NotFound, HttpStatusCode.Unauthorized);
    }
    ...
    public async Task GetDocumentsByEntity_MultipleEntityTypes_EachReturnsValid()
    {
        ...
        partner.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.Unauthorized);
        contact.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.Unauthorized);
        interaction.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.Unauthorized);
    }
    ...
    {
        var response = await _client.GetAsync("/api/document/download/999999");
        response.StatusCode.Should().BeOneOf(HttpStatusCode.NotFound, HttpStatusCode.Unauthorized);
    }
    ...
    {
        var body = new { data = "文档内容 李明", filename = "Unicode" };
        var response = await _client.PostAsJsonAsync("/api/document/generate-document", body, JsonOptions);
        response.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.BadRequest, HttpStatusCode.InternalServerError, HttpStatusCode.Unauthorized);
    }
}
