...
public class DocumentSecurityAndConcurrencyTests : IClassFixture<PAOWebApplicationFactory<Program>>
{
    ...
    public async Task PutDocument_Concurrent_HandlesGracefully()
    {
        ...
        var results = await Task.WhenAll(tasks);
        results.Should().AllSatisfy(r => r.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.NotFound, HttpStatusCode.BadRequest, HttpStatusCode.Unauthorized));
    }
