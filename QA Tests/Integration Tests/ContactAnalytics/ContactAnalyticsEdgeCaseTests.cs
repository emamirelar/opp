...
public class ContactAnalyticsEdgeCaseTests : IClassFixture<PAOWebApplicationFactory<Program>>
{
    ...
    public async Task GetContactEngagementTrends_WithMonths1_MinimumMonths_Returns200()
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<JsonElement>(JsonOptions);
        result.TryGetProperty("data", out _).Should().BeTrue();
        result.TryGetProperty("months", out var m).Should().BeTrue();
    ...
    public async Task GetRecentlyActiveContacts_WithDays1_ShortestPeriod_Returns200()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var result = await response.Content.ReadFromJsonAsync<JsonElement>(JsonOptions);
        result.TryGetProperty("days", out var d).Should().BeTrue();
    ...
    public async Task GetRecentlyActiveContacts_WithDays365_LongestPeriod_Returns200()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var result = await response.Content.ReadFromJsonAsync<JsonElement>(JsonOptions);
        result.TryGetProperty("sortBy", out var sb).Should().BeTrue();
    ...
    public async Task GetContactGrowthTrends_WithPeriodDaily_Returns200()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var result = await response.Content.ReadFromJsonAsync<JsonElement>(JsonOptions);
        result.TryGetProperty("period", out var p).Should().BeTrue();
    ...
    public async Task GetMostActiveContacts_WithTimeframe7d_ShortestTimeframe_Returns200()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var result = await response.Content.ReadFromJsonAsync<JsonElement>(JsonOptions);
        result.TryGetProperty("timeframe", out var tf).Should().BeTrue();
    ...
    public async Task GetContactsByGeographicRegion_AllPeriods_Returns200()
    {
        ...
        foreach (var period in periods)
        {
            ...
            response.StatusCode.Should().Be(HttpStatusCode.OK, $"period={period} should be accepted");
        }
    }
}
