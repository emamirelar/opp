...
namespace UNOPS.PAO.Tests.Integration.Controllers
{
    ...
    public class PartnerControllerEdgeCaseTests : IClassFixture<PAOWebApplicationFactory<Program>>
    {
        ...
        {
            // Act
            var response = await _client.GetAsync("/api/partner?search=合作*");
        ...
        public async Task CreatePartner_RtlText_HandlesGracefully()
        {
            // Arrange
            var rtlPartner = new { 
                Name = "\u202Eشريك الاسم\u202C", // RTL override characters
                Status = "Active"
            };
        ...
        public async Task CreatePartner_CombiningMarks_HandlesGracefully()
        {
            ...
            var diacriticPartner = new { 
                Name = "Pârtñér Ñámè", // Combining diacritical marks
                Status = "Active"
            };
        ...
        public async Task ImportPartners_MixedLineEndings_ParsesCorrectly()
        {
            ...
            // Assert
            response.StatusCode.Should().BeOneOf(HttpStatusCode.OK, HttpStatusCode.BadRequest, HttpStatusCode.UnsupportedMediaType, HttpStatusCode.Unauthorized);
        }
