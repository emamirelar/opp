...
public class PermissionControllerTests : IntegrationTestBase
{
    ...
    public async Task GetAllPermissions_AuthenticatedAdmin_ReturnsAllPermissions()
    {
        ...
        var permissions = await response.Content.ReadFromJsonAsync<List<dynamic>>();
        permissions.Should().NotBeNull("because permissions list should be returned");
    }
    ...
    public async Task GetPermissionById_ExistingPermission_ReturnsPermission()
    {
        ...
        // Act
        var response = await client.GetAsync($"/api/admin/permissions/{permissionId}");

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because existing permission should be found");
        var permission = await response.Content.ReadFromJsonAsync<dynamic>();
        permission.Should().NotBeNull("because permission details should be returned");
    }
    ...
    public async Task CreatePermission_ValidData_ReturnsCreatedPermission()
    {
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/admin/permissions", newPermission);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.Created, "because valid permission should be created");
        var createdPermission = await response.Content.ReadFromJsonAsync<dynamic>();
        createdPermission.Should().NotBeNull("because created permission should be returned");
    }
    ...
    public async Task CreatePermission_DuplicateName_ReturnsConflict()
    {
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/admin/permissions", duplicatePermission);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.Conflict, "because duplicate permission name should be rejected");
    }
    ...
    public async Task UpdatePermission_ExistingPermission_ReturnsSuccess()
    {
        ...
        // Act
        var response = await client.PutAsJsonAsync($"/api/admin/permissions/{permissionId}", updateData);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because existing permission should be updated");
        var updatedPermission = await response.Content.ReadFromJsonAsync<dynamic>();
        updatedPermission.Should().NotBeNull("because updated permission should be returned");
    }
    ...
    public async Task DeletePermission_UnusedPermission_ReturnsSuccess()
    {
        ...
        // Act
        var response = await client.DeleteAsync($"/api/admin/permissions/{permissionId}");

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.NoContent, "because unused permission should be deleted");
    }
    ...
    public async Task DeletePermission_PermissionInUse_ReturnsBadRequest()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "because permission in use cannot be deleted");
    }
    ...
    public async Task GetPermissionsByCategory_ValidCategory_ReturnsFilteredPermissions()
    {
        ...
        // Act
        var response = await client.GetAsync($"/api/admin/permissions?category={category}");

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because category filtering should be supported");
        var permissions = await response.Content.ReadFromJsonAsync<List<dynamic>>();
        permissions.Should().NotBeNull("because filtered permissions should be returned");
    }
    ...
    public async Task CreatePermission_InvalidName_ReturnsBadRequest()
    {
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/admin/permissions", invalidPermission);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "because invalid permission name should be rejected");
    }
    ...
    public async Task CreatePermission_MissingDescription_ReturnsBadRequest()
    {
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/admin/permissions", permissionWithoutDescription);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "because description is required");
    }
    ...
    public async Task AssignPermissionToRole_ValidRoleAndPermission_ReturnsSuccess()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because valid permission should be assigned to role");
    }
    ...
    public async Task RemovePermissionFromRole_AssignedPermission_ReturnsSuccess()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because assigned permission should be removed from role");
    }
    ...
    public async Task GetRolePermissions_RoleWithPermissions_ReturnsAllPermissions()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because role permissions should be retrievable");
        var permissions = await response.Content.ReadFromJsonAsync<List<dynamic>>();
        permissions.Should().NotBeNull("because role permissions should be returned");
    }
    ...
    public async Task GetRolePermissions_ChildRole_IncludesParentPermissions()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because child role permissions should include parent permissions");
        var permissions = await response.Content.ReadFromJsonAsync<List<dynamic>>();
        permissions.Should().NotBeNull("because inherited permissions should be returned");
    }
    ...
    public async Task AssignPermissionToRole_NonExistentPermission_ReturnsNotFound()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.NotFound, "because non-existent permission should be rejected");
    }
    ...
    public async Task GetPermissionUsage_AssignedPermission_ReturnsUsageReport()
    {
        ...
        // Act
        var response = await client.GetAsync($"/api/admin/permissions/{permissionId}/usage");

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because permission usage should be retrievable");
        var usageReport = await response.Content.ReadFromJsonAsync<dynamic>();
        usageReport.Should().NotBeNull("because usage report should be returned");
    }
    ...
    public async Task GetPermissionAudit_ModifiedPermission_ReturnsAuditLog()
    {
        ...
        // Act
        var response = await client.GetAsync($"/api/admin/permissions/{permissionId}/audit");

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because audit log should be available");
    ...
    public async Task CheckUserHasPermission_UserWithPermission_ReturnsTrue()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because permission check should complete");
        var hasPermission = await response.Content.ReadFromJsonAsync<bool>();
        hasPermission.Should().BeTrue("because user should have the permission");
    }
    ...
    public async Task CheckUserHasPermission_UserWithoutPermission_ReturnsFalse()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because permission check should complete");
        var hasPermission = await response.Content.ReadFromJsonAsync<bool>();
        hasPermission.Should().BeFalse("because user should not have the permission");
    }
    ...
    public async Task GetEffectivePermissions_AuthenticatedUser_ReturnsAllPermissions()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because effective permissions should be retrievable");
        var permissions = await response.Content.ReadFromJsonAsync<List<dynamic>>();
        permissions.Should().NotBeNull("because combined role permissions should be returned");
    }
    ...
    public async Task CheckPermission_Performance_CompletesWithin50ms()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because permission check should succeed");
    ...
    public async Task CheckPermission_SecondCall_UsesCachedResult()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because cached check should succeed");
    ...
    public async Task BulkAssignPermissionsToRole_ValidPermissions_ReturnsSuccess()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because bulk permission assignment should succeed");
    }
    ...
    public async Task BulkRemovePermissionsFromRole_AssignedPermissions_ReturnsSuccess()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because bulk permission removal should succeed");
    }
    ...
    public async Task CopyPermissions_BetweenRoles_ReturnsSuccess()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK, "because permission copy should succeed");
    }
    ...
    public async Task BulkAssignPermissions_InvalidPermission_ReturnsBadRequest()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "because invalid permission should cause failure");
    }
    ...
    public async Task BulkAssignPermissions_PartialFailure_RollsBack()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "because atomic operation should fail entirely");
    ...
    public async Task GetPermissions_NonAdminUser_ReturnsForbidden()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden, "because non-admin users cannot manage permissions");
    }
    ...
    public async Task AssignPermissionToSelf_HigherPermission_ReturnsForbidden()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden, "because users cannot elevate their own permissions");
    }
    ...
    public async Task DeletePermission_SystemPermission_ReturnsBadRequest()
    {
        ...
        // Act
        var response = await client.DeleteAsync($"/api/admin/permissions/{systemPermissionId}");

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "because system permissions are protected");
    }
    ...
    public async Task CreatePermission_InvalidCategory_ReturnsBadRequest()
    {
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/admin/permissions", permissionWithInvalidCategory);

        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest, "because invalid category should be rejected");
    }
