...
public class ConfigurationControllerTests : IClassFixture<PAOWebApplicationFactory<Program>>
{
    ...
    public async Task GetConfiguration_NoAuthRequired_Succeeds()
    {
        ...
        client.DefaultRequestHeaders.Clear();
        var response = await client.GetAsync("/api/configuration");
        response.StatusCode.Should().Be(HttpStatusCode.OK, "configuration endpoint does not require auth");
    }
