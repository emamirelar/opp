...
test.describe('Data Persistence - Page Load Integrity', () => {
  ...
  test('DPR-001: Partner detail retains data after page refresh', async ({ page }) => {
    ...
    const title = partnerPage.getByTestId('partner-title').first();
    ...
    const refreshedTitle = partnerPage.getByTestId('partner-title').first();
  ...
  test('DPR-002: Contact detail retains data after page refresh', async ({ page }) => {
    ...
    const title = contactPage.getByTestId('contact-title').first();
    await waitForVisible(title);
    const titleText = await title.textContent();
    expect(titleText?.trim().length).toBeGreaterThan(0);
    ...
    const refreshedTitle = contactPage.getByTestId('contact-title').first();
    await waitForVisible(refreshedTitle);
  ...
  test('DPR-003: Interaction detail retains data after page refresh', async ({ page }) => {
    ...
    const header = interactionPage.getByTestId('interaction-detail-header').first();
    await waitForVisible(header, 15000);
    const title = interactionPage.getByTestId('interaction-title').first();
    ...
    const refreshedHeader = interactionPage.getByTestId('interaction-detail-header').first();
    await waitForVisible(refreshedHeader, 15000);
    const refreshedTitle = interactionPage.getByTestId('interaction-title').first();
    expect(await refreshedTitle.textContent()).toBe(titleText);
  });
  ...
  test('DPR-004: Opportunity detail retains data after page refresh', async ({ page }) => {
    ...
    const title = opportunityPage.getByTestId('opportunity-title').first();
    await waitForVisible(title);
    const titleText = await title.textContent();
    expect(titleText?.trim().length).toBeGreaterThan(0);
    ...
    const refreshedTitle = opportunityPage.getByTestId('opportunity-title').first();
    await waitForVisible(refreshedTitle);
    expect(await refreshedTitle.textContent()).toBe(titleText);
  });
});
...
test.describe('Data Persistence - CRUD Operations', () => {
  ...
  test('DPR-009: Edit partner button is accessible on partner detail', async ({ page }) => {
    ...
    await waitForVisible(partnerPage.editButton, 15000);
    expect(await partnerPage.header.isVisible()).toBe(true);
    expect(await partnerPage.editButton.isVisible()).toBe(true);
  });
