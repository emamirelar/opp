...
export class PermissionService {
  ...
  /**
   * Check if a user has permission for an entity action
   */
  canPerformEntityAction(entityName: string, action: string): Observable<boolean> {
    return this.getConfig().pipe(
      switchMap(config => {
        // Find the entity in the config
        const entity = config.entities.find(e => e.name.toLowerCase() === entityName.toLowerCase());
        if (!entity) {
          console.warn(`[PERMISSION-SERVICE] Entity not found in permission config: ${entityName}`);
          return of(false);
        }
        ...
        if (!allowedRoles) {
          console.warn(`[PERMISSION-SERVICE] Action not found for entity: ${entityName}.${action}`);
          return of(false);
        }
