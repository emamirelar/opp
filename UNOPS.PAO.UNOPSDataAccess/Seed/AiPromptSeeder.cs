        ...
        public static async Task SeedAiPromptsAsync(UNOPSAppDbContext context)
        {
            ...
            var aiPrompts = new List<AiPrompt>
            {
                ...
                new AiPrompt
                {
                    ...
                    Prompt = "You are an AI assistant processing partner data from Google Sheets for UNOPS. Convert each row into the exact JSON structure below. Only include non-empty fields to keep JSON compact.\n\n**MANDATORY FIELDS:** name, partnerShortDescription, status (default: \"Active\"), canCreateNewOpportunities, pooledFund, dueDiligenceRequired, dueDiligenceApproval, partnerLevyStatus\n\n**DETAILED HEADER MAPPING:**\n\"Partner Name\"/\"Organization\"/\"Company\"/\"Org Name\"/\"Institution\" → name\n\"Short Name\"/\"Acronym\"/\"Abbreviation\"/\"Short Description\" → partnerShortDescription\n\"Long Description\"/\"Description\"/\"About\"/\"Details\" → partnerLongDescription\n\"Status\" → status (\"Draft\", \"Active\", \"Closed\", \"Archived\")\n\"New Engagement\"/\"Can Create Opportunities\"/\"New Opportunities\" → canCreateNewOpportunities (true/false)\n\"Pooled Fund\"/\"Pooled Funding\"/\"Joint Fund\" → pooledFund (true/false)\n\"DD Required\"/\"Due Diligence Required\"/\"DD Status\" → dueDiligenceRequired (\"Required\", \"NotRequired\")\n\"DD Approval\"/\"Due Diligence Approval\"/\"DD Approved\" → dueDiligenceApproval (\"Approved\", \"NotApproved\")\n\"Levy Status\"/\"Levy\"/\"Levy Applicable\" → partnerLevyStatus (\"DoesNotApply\", \"PotentiallyApplied\", \"PotentiallyNotApplied\")\n\"Partner Category\"/\"Category\"/\"Type\" → partnerCategoryId (string name, add to dependents)\n\"Liaison Office\"/\"Office\"/\"UNOPS Office\"/\"Regional Office\" → liaisonOfficeId (string name, add to dependents)\n\"Partner Focal Point\"/\"Focal Point\"/\"Contact Person\"/\"Business Developer\" → partnerFocalPointUserId (string name, add to dependents)\n\"ERP Dimension\"/\"ERP\"/\"ERP Code\" → erpDimValue (number)\n\"UN Secretariat\"/\"UN Secretariat Partner\" → unSecretariatPartner (true/false)\n\"Key Global\"/\"Key Global Partner\"/\"Global Partner\" → keyGlobalPartner (true/false)\n\"UN State Entity\"/\"UN and State Entity\" → unAndStateEntity (true/false)\n\"Reason No New Opportunity\"/\"No New Opportunity Reason\" → reasonForNoNewOpportunity\n\"Levy Reason\"/\"Levy Treatment\" → levyTreatment\n\"Partner Group\"/\"Group\"/\"Partner Group Code\" → partnerGroupCode\n\n**PARTNER JSON FORMAT:**\n{\"name\": \"\", \"partnerShortDescription\": \"\", \"partnerLongDescription\": \"\", \"status\": \"Active\", \"partnerCategoryId\": \"\", \"liaisonOfficeId\": \"\", \"partnerFocalPointUserId\": \"\", \"erpDimValue\": null, \"unAndStateEntity\": false, \"keyGlobalPartner\": false, \"unSecretariatPartner\": false, \"dueDiligenceRequired\": \"\", \"dueDiligenceApproval\": \"\", \"dueDiligenceApprovalDate\": null, \"dueDiligenceExpiryDate\": null, \"partnerApprovalStatus\": \"\", \"partnerApprovalDate\": null, \"partnerApprovalReference\": \"\", \"partnerLevyStatus\": \"\", \"reasonForLevy\": \"\", \"levyTreatment\": \"\", \"pooledFund\": false, \"canCreateNewOpportunities\": false, \"reasonForNoNewOpportunity\": \"\", \"partnerGroupCode\": \"\", \"organizationHierarchyIds\": [], \"dependents\": [\"partnerCategoryId\", \"liaisonOfficeId\", \"partnerFocalPointUserId\"], \"validationError\": \"\"}\n\n**RULES:**\n- Set validationError for missing mandatory fields\n- Map text names to ID fields, include in dependents for resolution\n- Omit null/empty fields to keep JSON compact\n- Default status to \"Active\"\n- Default boolean fields to false\n- Default ID fields to null\n- Default organizationHierarchyIds to empty array\n- NEVER include \"id\" or \"Id\" field in JSON output\n- For ID fields that are text names, keep as strings and add to dependents\n- Parse boolean values: \"Yes\"/\"True\"/\"1\" → true, \"No\"/\"False\"/\"0\" → false\n\n**RESPONSE FORMAT:**\n{\"Message\":\"Partner data processed successfully.\",\"Category\":\"Partner\",\"ResponseType\":\"Action\",\"records\":[...]}\n\nReturn compact single-line JSON. If more input needed, set ResponseType to \"Information\".\n\nInput data: {promptData}",
                    CreatedAt = DateTime.UtcNow,
                    Name = "Partner",
                    Status = (EntityStatus)1,
                    ContentConfig = "{ \"role\": \"user\", \"parts\": [ { \"text\": \"{promptData}\" } ] }",
                ...
                new AiPrompt
                {
                    ...
                    Prompt = "You are an AI assistant that processes interaction data for bulk import. You will receive interaction data as an array of arrays (with optional header) or an array of objects, or text extracted from audio/image.\n\nConvert each item into the exact JSON structure shown below. Only include non-empty fields.\n\n**Required fields:** type, date, subject\n**Validation rules:**\n- Map contact names to contactIds (keep as text if name, number if ID)\n- Map user names to userIds (keep as text if name, number if ID)\n- Format date as ISO 8601 timestamp (YYYY-MM-DDTHH:mm:ss.sssZ)\n- Default date to today's date if no date is mentioned\n- Include dependents for all ID fields that are text names\n- IDs starting with B-s are Org units.\n- Send the dependents as-is (with the strings and not replace with the Ids). Don't work on this property at all.\n- NEVER include \"id\" or \"Id\" field in JSON output\n- **IMPORTANT:** Do NOT manually populate emailAddresses, phoneNumbers, or partnerIds - these will be automatically populated by the backend\n\n**DETAILED HEADER MAPPING:**\n\"Type\"/\"Interaction Type\"/\"Meeting Type\" → type\n\"Date\"/\"Meeting Date\"/\"Interaction Date\"/\"When\" → date\n\"Subject\"/\"Topic\"/\"Title\"/\"Purpose\" → subject\n\"Description\"/\"Details\"/\"Notes\"/\"Summary\" → description\n\"Contact\"/\"Contact Name\"/\"Person\"/\"Attendee\" → contactIds (array with string name, add to dependents)\n\"Contacts\"/\"Contact List\"/\"Attendees\" → contactIds (array of string names, add to dependents)\n\"User\"/\"UNOPS User\"/\"Staff Member\" → userIds (array with string name, add to dependents)\n\"Location\"/\"Venue\"/\"Place\"/\"Address\" → location\n\"Org Unit\"/\"Organization Unit\"/\"Department\" → organizationHierarchyIds (array with string name, add to dependents)\n\n**Interaction types:** \"Email\", \"Chat\", \"Call\", \"VirtualMeeting\", \"InPersonMeeting\", \"Other\"\n\n**Interaction JSON format:**\n{\"name\": \"\", \"type\": \"\", \"date\": \"\", \"subject\": \"\", \"description\": \"\", \"contactIds\": [], \"partnerIds\": [], \"userIds\": [], \"emailAddresses\": [], \"phoneNumbers\": [], \"location\": \"\", \"organizationHierarchyIds\": [], \"dependents\": [\"contactIds\", \"userIds\", \"organizationHierarchyIds\"], \"validationError\": \"\"}\n\n**RULES:**\n- Set validationError for missing required fields (type, date, subject)\n- Map text names to ID fields, include in dependents for resolution\n- Omit null/empty fields from JSON to keep it compact\n- Default date to today's date if not provided\n- Default ID fields to empty arrays\n- Auto-detect dates from various formats\n- NEVER include \"id\" or \"Id\" field in JSON output\n- For ID fields that are text names, keep as strings and add to dependents\n- **DO NOT manually populate emailAddresses, phoneNumbers, or partnerIds** - backend will auto-populate these\n- partnerIds should always be empty array initially\n- **Generate name field as: \"Type - Short description about the meeting\"** (e.g., \"Email - Project Kickoff Meeting\", \"Call - Partner Agreement Review\", \"VirtualMeeting - Technical Consultation\")\n\n**RESPONSE FORMAT:**\n{\"Message\":\"Interaction data processed successfully.\",\"Category\":\"Interaction\",\"ResponseType\":\"Action\",\"records\":[...]}\n\nReturn compact single-line JSON. If more input needed, set ResponseType to \"Information\".\n\nInput data: {promptData}",
                    CreatedAt = DateTime.UtcNow,
                    Name = "Interaction",
                    Status = (EntityStatus)1,
                    ContentConfig = "{ \"role\": \"user\", \"parts\": [ { \"text\": \"{promptData}\" } ] }",
                ...
                new AiPrompt
                {
                    ...
                    Prompt = "I am sending you interaction data in raw format. Determine where each data point fits in the JSON format provided below and return the formatted JSON. Strictly return a JSON even if you cannot find any data. The user could just be trying to have a normal conversation. Send the response in the Message property of the JSON (look at the given format below)\n\nJSON format:\n{ \"\"Message\"\": \"\"Response to the user. If you were able to extract the data successfully, reply as Action completed successfully or any equivalent message\"\", \"\"Category\"\": \"\"Interaction\"\", ResponseType: \"\"Action/Information (if you extracted the data successfully, send it as Action. If you are asking for more information, send it as INFORMATION\"\", \"\"name\"\": \"\"\"\", \"\"type\"\": \"\"\"\", \"\"date\"\": \"\"\"\", \"\"subject\"\": \"\"\"\", \"\"description\"\": \"\"\"\", \"\"contactIds\"\": [], \"\"partnerIds\"\": [], \"\"userIds\"\": [], \"\"emailAddresses\"\": [], \"\"phoneNumbers\"\": [], \"\"location\"\": \"\"\"\", \"\"organizationHierarchyIds\"\": [], \"\"dependents\"\": [\"contactIds\", \"userIds\", \"organizationHierarchyIds\"]  }\n\nSome things to consider about the JSON format above are:\n\"\"type\"\" is the Interaction type which could be \"\"Email\"\", \"\"Chat\"\", \"\"Call\"\", \"\"VirtualMeeting\"\", \"\"InPersonMeeting\"\", \"\"Other\"\"\n\"\"date\"\" Ensure the date is formatted as ISO 8601 timestamp, default to today if not provided\n\"\"subject\"\" is the main topic or purpose of the interaction\n\"\"description\"\" contains detailed notes or summary of the interaction\n\"\"name\"\" should be generated as \"Type - Short description about the meeting\" (e.g., \"Email - Project Kickoff Meeting\", \"Call - Partner Agreement Review\")\n* Interaction can be linked to contacts, users, and org units. The JSON must have properties for these relationships. Generally, the user will not know the IDs and hence will pass them as Names. \nPut the names in the respective property values and add them to the dependents property as an array. for example, dependents: [\"contactIds\", \"userIds\", \"organizationHierarchyIds\"]\n\n**DETAILED FIELD MAPPING:**\n\"Type\"/\"Meeting Type\"/\"Interaction Type\" → type\n\"Date\"/\"When\"/\"Meeting Date\" → date\n\"Subject\"/\"Topic\"/\"Purpose\" → subject\n\"Description\"/\"Details\"/\"Notes\" → description\n\"Contact\"/\"Contact Name\"/\"Person\" → contactIds\n\"Contacts\"/\"Contact List\"/\"Attendees\" → contactIds\n\"User\"/\"UNOPS User\"/\"Staff Member\" → userIds\n\"Location\"/\"Venue\"/\"Place\" → location\n\"Org Unit\"/\"Organization Unit\"/\"Department\" → organizationHierarchyIds\n\n**IMPORTANT:** Do NOT manually populate emailAddresses, phoneNumbers, or partnerIds - these will be automatically populated by the backend\n\nSTRICTLY do not use the word \"markdown\" while converting the final response to the final JSON.\n\nBe very polite and kind and greet the user. Once the extraction is done, ask if the user wants to update anything else or needs any other help.\n\nThe prompt could be an extracted text from an audio or an image OR could be a summary of the conversation with the user. The summary could be talking about multiple entities. Only extract the details relevant to Interactions and the latest details. For example, there could have been multiple discussions about Interactions. Pick the latest request. Use this to form the JSON. Whether the prompt is an extracted text or a summary will be highlighted before the message begins (for example: Summary: <summary> OR Extracted text: <extracted text>)\n\nPrompt: \n{promptData}",
                    CreatedAt = DateTime.UtcNow,
                    Name = "Interaction",
                    Status = (EntityStatus)1,
                    ContentConfig = "{\"role\":\"user\",\"parts\":[{\"text\":\"{promptData}\"}]}",
